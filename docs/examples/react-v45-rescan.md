# DST v4.5 Rescan — facebook/react

**DST V4.5 Diagnostic — Live Scan Results**  
*Scanned March 2026 · DST Framework v4.5 · SSRN 6434119*  
...
This is the third DST scan of facebook/react. V4 ran in early March 2026 on react v19.3.0 (1,864 files). V4.5 runs now on the current main branch (4,202 JS/TS source files — the scanner now reaches more of the monorepo). The version under scan: commit `ed69815`, the latest main as of March 23 2026.
...
```
node tools/dst-scanner.js /tmp/react-scan
```

*(Internally: `DST_SCAN_DIR=/tmp/react-scan node tools/dst-action.js`)*

> **Note on header:** The `DST DIAGNOSTIC — v4` banner is a hardcoded string in `buildConsoleOutput` in `dst-action.js`. It does not auto-increment with patch versions. This is the verbatim raw output from the v4.5 tool.
...
```
DST_DATA_SCALE=hyperscale node tools/dst-scanner.js /tmp/react-scan
```

*(Internally: `DST_SCAN_DIR=/tmp/react-scan DST_DATA_SCALE=hyperscale node tools/dst-action.js`)*

> **Note on header:** Same as Run 1 — the `DST DIAGNOSTIC — v4` banner is hardcoded. The v4.5 σ scaling is active (`SIGMA_SCALE=4.0`); the banner does not reflect this.
...
### Why the two runs produce identical console output

The console output is identical because React's Θ is pinned to 100 by ρ-dominance regardless of σ scaling.
...
`σ_eff` is computed from raw counts (`sigmaCount`, `kappaCount`) and is not affected by `DST_DATA_SCALE` — hence `4.8` is unchanged between runs.
...
| Metric | V4 (March 2026, v19.3.0) | V4.5 (March 2026, main ed69815) | Change |
|--------|--------------------------|----------------------------------|--------|
...
| σ findings | 8 | 24 | +16 (+200%) — more files exposed more amplifiers |
...
The regime and Θ are structurally identical between v4 and v4.5. React is Elastic in both versions. The larger σ_eff (4.8 vs 1.6) reflects the wider scan surface, not a deterioration of the codebase.
...
React v4.5 scan confirms the v4 conclusion: **Θ = 100, Elastic, structurally sound**. The expanded file surface (+125% files) revealed more κ and σ instances, but ρ grew proportionally faster, keeping the regime stable. The three new v4.5 capabilities found nothing of alarm:
...
The finding that warrants attention is σ_eff rising from 1.6 → 4.8: three times as many stress amplifiers are now masked by κ. The 22 unbounded growth patterns and 2 N+1 patterns are real structural risks that the React team's κ discipline is currently containing. If κ were to be removed without addressing those σ findings first, the effective stress would become visible immediately.
